...
fn main() {
	...
	let data = match fs::read_to_string(&args.filename) {
		Ok(v) => v,
		Err(e) => {
			eprintln!("{} failed to read from '{}' : {:?}", "Error".red().bold(), args.filename, e);
			std::process::exit(1);
		}
	};
	let new_data = match replace(&args.target, &args.replacement, &data) {
		Ok(v) => v,
		Err(e) => {
	...
	match fs::write(&args.output, &new_data) {
		Ok(_) => {}
		Err(e) => {
			eprintln!("{} failed to write to '{}' : {:?}", "Error".red().bold(), args.filename, e);
			std::process::exit(1);
		}
	}
}
